...
struct HD_USTC_CG_GL_API ShaderDesc {
   ...
   private:
    ...
    friend HD_USTC_CG_GL_API ShaderHandle createShader(const ShaderDesc& desc);
...
using ShaderHandle = std::shared_ptr<ShaderResource>;
HD_USTC_CG_GL_API ShaderHandle createShader(const ShaderDesc& desc);
...
HD_USTC_CG_GL_API TextureHandle createTexture(const TextureDesc& desc);
